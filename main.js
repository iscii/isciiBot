...
//get command files
const normCmdFiles = fs
	.readdirSync("./commands/normal")
	.filter((file) => file.endsWith(".js"));
const gameCmdFiles = fs
	.readdirSync("./commands/games")
	.filter((file) => file.endsWith(".js"));
...
async function cmdGeneral(msg) {
	...
	if (msg.content.includes(prefix)) {
		...
		if (cmd == "join") {
			...
			else msg.channel.send("You are not in a voice channel");
			return;
		}
