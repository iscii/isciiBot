...
client.on("message", (msg) => {
    ...
    if(msg.guild && msg.content[0] == prefix){
        switch (args[0]){ //learn the switch statement. Note: If you set the argument to a string and case the string, it'll spam the function 5 times.
            ...
            break;
            case "serveremotelist":
                console.log("serveremotelist");
            ...
            case "allemotelist":
                ...
                client.guilds.forEach(e => {
                    //console.log(e.emojis);
                    var emoteList = (e.emojis.map(emoji => emoji + emoji.name));
                    for(i = 3; i < emoteList.length; i += 4){
                        emoteList.splice(i, 0, "\n");
                    }
                    emoteList = emoteList.join("");
                    if(emoteList.length > 2000){
                        elist1 = emoteList.substring(0, 1999);
                        elist2 = emoteList.substring(2000, emoteList.length - 1);
                        msg.channel.send("**" + e.name + "**" + "\n" + elist1 + elist2);
                    }
                    else msg.channel.send("**" + e.name + "**" + "\n" + emoteList);
                })
                //emoteList.join(" ");
                /*
                for(i = 5; i < emoteList.length; i += 6){
                    emoteList.splice(i, 0, "\n");
                }
                emoteList*/
                /*console.log(emoteList);
                msg.channel.send(emoteList.substring(0, 2000));*/
                //console.log(emoteList);
                /*console.log(client.guilds.forEach(k =>
                    k.emojis.map(emoji => emoji)
                ))*/
                //console.log(client.guilds.emojis.map(emoji => emoji + " ~ " + emoji.name));
            case "say":
                console.log("say");
