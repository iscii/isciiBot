        ...
        switch (args[0]){ //learn the switch statement. Note: If you set the argument to a string and case the string, it'll spam the function 5 times.
            ...
            case "emotelist":
                ...
                function checkForSplit(){
                    var index = 0;
                    for(i = 0; i < Math.ceil(emoteList.length / 40); i++){
                        if(emoteList.length - index < 40){
                            message[i] = emoteList.slice(index, emoteList.length);
                        }
                        else{
                            message[i] = emoteList.slice(index, index + 40)
                            index += 40;
                        }
                    }
                    for(i in message){
                ...
                if(!args[1]){ //defaults to cuurrent guild
                    ...
                    if(emoteList.length > 2000){
                        ...
                        var sec1 = emoteList.substring(0, 1999);
                        var sec2 = emoteList.substring(2000, emoteList.length);
                        //if(){ //if it's cut, and brackets are found, then check for more brackets until the next \n and set that to carry. then add carry to sec2.

                        //}
                        for(i = 0; i < 50; i++){
                            if(emoteList.substring(emoteList.length - 50, emoteList.length)[i] == "<")
                        }
                        var carry;
                        msg.channel.send(sec1);
                        return msg.channel.send(sec2);
                    }
                    msg.channel.send(emoteList); 
                }*/
            break;
